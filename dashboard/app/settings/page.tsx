...
import { fetchIntegrationCatalog, fetchWorkflowDetails, fetchWorkflows, removeIntegrationCredentials, saveIntegrationCredentials, updateWorkflowIntegrations, verifyConnection } from '../lib/api';
...
const ALLOWED: Record<string, string[]> = {
  ...
  x: ['content_engine'], linkedin: ['content_engine'], meta: ['content_engine', 'instagram_comments'], discord: ['content_engine'], runpod: [],
};
...
export default function SettingsPage() {
  ...
  const linkedCount = useMemo(() => new Set(integrations.flatMap((item) => item.linked_workflows)).size, [integrations]);
    ...
    <div className="space-y-7 pb-16">
      ...
      <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        {integrations.map((connection) => {
          ...
          return <article key={connection.id} className="surface-card group rounded-2xl p-5"><div className="flex items-start justify-between gap-3"><span className={`grid h-11 w-11 place-items-center rounded-2xl border ${verified ? 'border-emerald-300/20 bg-emerald-300/8 text-emerald-300' : 'border-white/10 bg-white/5 text-slate-400'}`}><PlugZap className="h-5 w-5" /></span><span className={`rounded-full border px-2.5 py-1 text-[10px] font-black uppercase tracking-wide ${verified ? 'border-emerald-300/20 bg-emerald-300/8 text-emerald-300' : connection.configured ? 'border-amber-300/20 bg-amber-300/8 text-amber-300' : 'border-white/10 bg-white/5 text-slate-500'}`}>{connection.status.replaceAll('_',' ')}</span></div><h2 className="mt-5 font-bold text-slate-100">{connection.display_name}</h2><p className="mt-1 min-h-10 text-sm leading-5 text-slate-500">{connection.description}</p><div className="mt-4 flex flex-wrap gap-1.5">{connection.linked_workflows.map((id) => <span key={id} className="rounded-lg bg-cyan-300/8 px-2 py-1 text-[10px] font-bold text-cyan-300">{workflows.find((item) => item.id === id)?.display_name ?? id}</span>)}{connection.linked_workflows.length === 0 && <span className="text-xs text-slate-600">Not linked to an automation</span>}</div><button onClick={() => openEditor(connection)} className="mt-5 flex h-10 w-full items-center justify-center gap-2 rounded-xl border border-white/10 bg-white/5 text-xs font-bold text-slate-200 hover:border-cyan-300/20 hover:bg-cyan-300/8"><LockKeyhole className="h-4 w-4" />{connection.configured ? 'Manage securely' : 'Configure connection'}</button></article>;
        })}
      </section>
