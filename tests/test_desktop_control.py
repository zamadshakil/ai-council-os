...
def test_recovery_starts_missing_panel_without_dbus_restart(monkeypatch) -> None:
    ...
    monkeypatch.setattr(desktop_control, "_spawn", lambda command: spawned.append(command))
    monkeypatch.setattr(desktop_control, "_terminate_failed_panel_helpers", lambda: [42])
    monkeypatch.setattr(
        desktop_control,
...
def test_desktop_launchers_are_made_executable_and_trusted(monkeypatch, tmp_path) -> None:
    ...
    assert commands == [
        ["gio", "set", str(desktop / name), "metadata::trusted", "true"]
        for name in desktop_control.DESKTOP_LAUNCHERS
    ]
